...
namespace WebRTC.Droid.Demo
{
    public class CaptureQualityController : Java.Lang.Object,SeekBar.IOnSeekBarChangeListener
    {
        ...
        public void OnProgressChanged(SeekBar seekBar, int progress, bool fromUser)
        {
            ...
            {
                maxCaptureBandwidth = Math.Max(maxCaptureBandwidth, format.Width * format.Height * format.MaxFramerate);
            }

            var bandwidthFraction = progress / 100f;
            
            
            bandwidthFraction = (float) (Math.Exp(ExpConstant * bandwidthFraction)/(Math.Exp(ExpConstant) -1 ));
            ...
            _captureFormatText.Text = context.GetString(Resource.String.format_description,_width,_height,_framerate);
